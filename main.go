...
FMT:
	...
	if *szstr == "" {
		...
		xW, xH := s[0], s[1]
		uW, _ := strconv.ParseUint(xW, 10, 32)
		uH, _ := strconv.ParseUint(xH, 10, 32)

		for _, f := range frames {
			mW, mH := uint(f.MaxWidth), uint(f.MaxHeight)

			if mW == uint(uW) && mH == uint(uH) {
				size = &f
				break
			}
		}
